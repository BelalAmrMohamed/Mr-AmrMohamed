...
## AI Chat
- Many icons are messed up or don't match the icons I want:
    - [x] Update the `ai-send-btn` icon, it's the wrong icon, it's currently a bad icon, colored badly, doesn't blend seemlesly in the `.ai-input-shell`.
    - [x] Update / fix the `.ai-sidebar-collapse-hover` icon; it's missing a path or something.
    - [x] Update / fix the delete icon in the `.ai-context-menu` similar, to the previous, it's almost the same, just seems to be missing a path.
...
- [x] The placeholder in the `.ai-input-shell` becomes misaligned after the first prompt.
